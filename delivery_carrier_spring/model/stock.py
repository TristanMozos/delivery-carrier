...
class StockPicking(models.Model):
    ...
    def _spring_shipment_request(self):
        ...
        aux_address = '%s %s %s' % (self.partner_id.street or '', self.partner_id.street2 or '', self.partner_id.street3 or '')
        adress_parts = aux_address.split(' ')
        aux = ''
        aux2 = None
        i=0
        for part in adress_parts:
            aux2 = aux
            aux = ('%s %s' % (aux, part.strip())).strip()
            if len(aux)>35:
                aux = part
                if i == 0:
                    self.partner_id.street = aux2
                elif i == 1:
                    self.partner_id.street2 = aux2
                elif i == 2:
                    self.partner_id.street3 = aux2
                i += 1
